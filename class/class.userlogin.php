...
class JAK_userlogin {
	...
	public static function jakWriteDeviceToken($userid, $device, $token) {
		...
		if (isset($pushid) && !empty($pushid)) {
			...
			$jakdb->insert("push_notification_devices", ["userid" => $userid, "ostype" => $device, "token" => $token, "lastedit" => $jakdb->raw("NOW()"), "created" => $jakdb->raw("NOW()")]);
		}
