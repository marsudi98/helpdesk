...
$similarlink = JAK_FAQ_URL;
$similarshort = 'a';
$similartitle = $jkl['hd125'];
...
if ($page1 == "a" && isset($page2) && is_numeric($page2) && jak_row_exist($page2, $jaktable2)) {
  ...
  if (JAK_CLIENTID) {
    if ($jakclient->getVar("faq_cat") == 0) {
        ...
        // Similar
        $similarart = $jakdb->select($jaktable2, ["id", "title", "content", "time"], ["AND" => ["id[!]" => $page2, "active" => 1, "content[~]" => $titlearray], "LIMIT" => 3]);
      ...
      if (!$jakdb->has($jaktable3, ["AND" => ["OR" => ["guesta" => 1, "id" => [$jakclient->getVar("faq_cat")]]]])) jak_redirect(JAK_rewrite::jakParseurl(JAK_FAQ_URL));

        // Categories
        $allcategories = $jakdb->select($jaktable3, ["id", "title"], ["AND" => ["id" => [$jakclient->getVar("faq_cat")], "active" => 1]]);

        // Similar
        $similarart = $jakdb->select($jaktable2, ["id", "title", "content", "time"], ["AND" => ["id[!]" => $page2, "catid" => [$jakclient->getVar("faq_cat")], "active" => 1, "content[~]" => $titlearray], "LIMIT" => 3]);
    ...
    if ($jakuser->getVar("faq_cat") == 0 || JAK_SUPERADMINACCESS) {
      ...
      // Similar
      $similarart = $jakdb->select($jaktable2, ["id", "title", "content", "time"], ["AND" => ["id[!]" => $page2, "active" => 1, "content[~]" => $titlearray], "LIMIT" => 5]);
      ...
      if (!$jakdb->has($jaktable3, ["OR" => ["guesta" => 1, "id" => [$jakuser->getVar("faq_cat")]]])) jak_redirect(JAK_rewrite::jakParseurl(JAK_FAQ_URL));
        ...
        // Similar
        $similarart = $jakdb->select($jaktable2, ["id", "title", "content", "time"], ["AND" => ["id[!]" => $page2, "catid" => [$jakuser->getVar("faq_cat")], "active" => 1, "content[~]" => $titlearray], "LIMIT" => 3]);
    ...
    if (!$jakdb->has($jaktable3, ["guesta" => 1])) jak_redirect(JAK_rewrite::jakParseurl(JAK_FAQ_URL));
      ...
      // Similar
      $similarart = $jakdb->select($jaktable2, ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.title", "faq_article.content", "faq_article.time"], ["AND" => ["faq_article.id[!]" => $page2, "faq_categories.guesta" => 1, "faq_article.active" => 1, "faq_article.content[~]" => $titlearray], "LIMIT" => 3]);
...
// We need only the categories
  ...
  if (JAK_CLIENTID) {
    ...
    if ($jakclient->getVar("faq_cat") == 0) {
      ...
      if ($getTotal != 0) {
        ...
        $pages->mid_range = JAK_FAQ_PAGINATION;
        $pages->items_per_page = JAK_FAQ_PAGE;
        ...
        // Get the result
        $allfaq = $jakdb->select("faq_article", ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.catid", "faq_article.lang", "faq_article.title", "faq_article.content", "faq_categories.class", "faq_article.time", "faq_categories.id(idcat)", "faq_categories.title(titlecat)"], ["AND" => ["faq_article.catid" => $page2, "faq_article.lang" => $BT_LANGUAGE, "faq_article.active" => 1],
    ...
    // Only for certain categories
      ...
      if ($getTotal != 0) {
        ...
        // Get the result
        $allfaq = $jakdb->select("faq_article", ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.catid", "faq_article.lang", "faq_article.title", "faq_article.content", "faq_article.time", "faq_categories.class", "faq_categories.id(idcat)", "faq_categories.title(titlecat)"], ["AND" => ["faq_article.catid" => $page2, "faq_article.lang" => $BT_LANGUAGE, "faq_article.catid" => [$jakclient->getVar("faq_cat")], "faq_article.active" => 1],
          "ORDER" => ["faq_article.dorder" => "DESC"],
          "LIMIT" => $pages->limit
          ]);
  ...
  // Can see all active articles
    ...
    if ($getTotal != 0) {
      ...
      // Get the result
      $allfaq = $jakdb->select("faq_article", ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.catid", "faq_article.lang", "faq_article.title", "faq_article.content", "faq_article.time", "faq_categories.class", "faq_categories.id(idcat)", "faq_categories.title(titlecat)"], ["AND" => ["faq_article.catid" => $page2, "faq_article.lang" => $BT_LANGUAGE, "faq_article.active" => 1],
          "ORDER" => ["faq_article.dorder" => "DESC"],
          "LIMIT" => $pages->limit
        ]);
  ...
  // Can see categories for guests
    ...
    if ($getTotal != 0) {
      ...
      // Get the result
      $allfaq = $jakdb->select("faq_article", ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.catid", "faq_article.lang", "faq_article.title", "faq_article.content", "faq_article.time", "faq_categories.class", "faq_categories.id(idcat)", "faq_categories.title(titlecat)"], ["AND" => ["faq_article.catid" => $page2, "faq_categories.guesta" => 1, "faq_article.lang" => $BT_LANGUAGE, "faq_article.active" => 1],
          "ORDER" => ["faq_article.dorder" => "DESC"],
          "LIMIT" => $pages->limit
        ]);
  ...
  if (isset($allfaq) && !empty($allfaq)) foreach ($allfaq as $val) {
      if ($val["catid"] != 0) $nav[$val["idcat"]] = array("idcat" => $val["idcat"], "titlecat" => $val["titlecat"]);
  }
  ...
  if (JAK_CLIENTID) {
    ...
    if ($jakclient->getVar("faq_cat") == 0) {
      ...
      if ($getTotal != 0) {
        ...
        // Get the result
        $allfaq = $jakdb->select("faq_article", ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.catid", "faq_article.lang", "faq_article.title", "faq_article.content", "faq_article.time", "faq_categories.class", "faq_categories.id(idcat)", "faq_categories.title(titlecat)"], ["AND" => ["faq_article.lang" => $BT_LANGUAGE, "faq_article.active" => 1],
          "ORDER" => ["faq_article.dorder" => "DESC"],
          "LIMIT" => $pages->limit
          ]);
    ...
    // Only for certain categories
      ...
      if ($getTotal != 0) {
        ...
        // Get the result
        $allfaq = $jakdb->select("faq_article", ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.catid", "faq_article.lang", "faq_article.title", "faq_article.content", "faq_article.time", "faq_categories.class", "faq_categories.id(idcat)", "faq_categories.title(titlecat)"], ["AND" => ["faq_article.lang" => $BT_LANGUAGE, "faq_article.catid" => [$jakclient->getVar("faq_cat")], "faq_article.active" => 1],
          "ORDER" => ["faq_article.dorder" => "DESC"],
          "LIMIT" => $pages->limit
          ]);
  ...
  // Can see categories for guests
    ...
    if ($getTotal != 0) {
      ...
      // Get the result
      $allfaq = $jakdb->select("faq_article", ["[>]faq_categories" => ["catid" => "id"]], ["faq_article.id", "faq_article.catid", "faq_article.lang", "faq_article.title", "faq_article.content", "faq_article.time", "faq_categories.class", "faq_categories.id(idcat)", "faq_categories.title(titlecat)"], ["AND" => ["OR" => ["faq_categories.guesta" => 1, "faq_article.catid" => 0], "faq_article.lang" => $BT_LANGUAGE, "faq_article.active" => 1],
          "ORDER" => ["faq_article.dorder" => "DESC"],
          "LIMIT" => $pages->limit
        ]);
