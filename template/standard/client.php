...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Login IN
	if (!empty($jkp['action']) && $jkp['action'] == 'login') {
	...
	// Forgot password
 	if (!empty($jkp['action']) && $jkp['action'] == 'forgot-password') {
	...
	// Register
	if (!empty($jkp['action']) && $jkp['action'] == 'register') {
				...
				if (isset($formfields) && !empty($formfields)) {
					foreach ($formfields as $v) {
						if (isset($jkp[$v]) && is_array($jkp[$v])) {
						    $joinval = join(',', $jkp[$v]);
							$jakdb->update($jaktable2, [$v => $joinval], ["id" => $lastid]);
						} else {
						    $jakdb->update($jaktable2, [$v => $jkp[$v]], ["id" => $lastid]);
						}
					}
				}
	...
	// Save Client
	if (!empty($jkp['action']) && $jkp['action'] == 'save_client') {
	...
	// Transfer payments
	if (!empty($jkp['action']) && $jkp['action'] == 'payment') {
...
if ($page1 == "rt") {
	...
	if (isset($page2) && isset($page3) && is_numeric($page2) && is_numeric($page3) && $jakdb->has($jaktable1, ["AND" => ["id" => $page2, "initiated" => $page3, "reminder" => 2]])) {
		...
		// Load the template
		include_once APP_PATH.'template/standard/tplblocks/rateticket.php';
...
if ($page1 == "edit") {
	...
	$custom_fields = jak_get_custom_fields($JAK_FORM_DATA, 1, false, $BT_LANGUAGE, false, false, false, false, $errorsp);

	// Load the template
	include_once APP_PATH.'template/standard/tplblocks/clientedit.php';
	...
	if (JAK_CLIENTID) {
		...
		$last5pay = $jakdb->select("subscriptions", ["[>]billing_packages" => ["package" => "id"]], ["subscriptions.id", "subscriptions.amount", "subscriptions.currency", "subscriptions.paidhow", "subscriptions.paidwhen", "subscriptions.success", "billing_packages.title"], ["subscriptions.clientid" => JAK_CLIENTID,
		    "ORDER" => ["subscriptions.paidwhen" => "DESC"],
		    "LIMIT" => 5
		]);
		...
		// Load the template
		include_once APP_PATH.'template/standard/tplblocks/dashboard.php';
	...
	if (JAK_USERID) {
		...
		// Load the template
		include_once APP_PATH.'template/standard/tplblocks/dashboardo.php';

	}

}

// Get the stuff when operator is not logged in but user is
} else {
	// Get the custom fields
	$custom_fields = jak_get_custom_fields($jkp, 1, false, $BT_LANGUAGE, false, false, false, true, $errorsreg);

	// Include the javascript file for results
	$js_file_footer = 'js_client.php';

	// Load the template
	include_once APP_PATH.'template/standard/tplblocks/client.php';
}
?>