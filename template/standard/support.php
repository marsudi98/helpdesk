      ...
      if (!$result) {
        ...
        if (!JAK_CLIENTID && isset($jkp['createaccount']) && $jkp['createaccount'] == 1 || !JAK_CLIENTID && JAK_TICKET_ACCOUNT) {
          ...
          if (!$clientid) {
            ...
            // Write the log file each time someone tries to login before
            JAK_base::jakWhatslog('', 0, $clientid, 12, $clientid, (isset($_COOKIE['WIOgeoData']) ? $_COOKIE['WIOgeoData'] : ''), $cemail, $_SERVER['REQUEST_URI'], $ipa, $valid_agent);
...
// Only if we have a valid department we call the rest
  if (isset($_SESSION["depinfo"]) && is_numeric($_SESSION["depinfo"])) {
    ...
    $custom_fields = jak_get_custom_fields($jkp, 2, $_SESSION["depinfo"], $jakclient->getVar("language"), false, false, true, false, $errors);
...
// The ticket is private and we are not the owner
  ...
  // Similar
    $similarart = $jakdb->select($jaktable2, ["id", "subject"], ["OR #sim" => ["AND #public" => ["id[!]" => $page2, "content[~]" => $titlearray, "private" => 0], "AND #private" => ["id[!]" => $page2, "content[~]" => $titlearray, "private" => 1, "clientid" => JAK_CLIENTID]], "LIMIT" => 5]);
  ...
  // Page Navigation
    ...
    $allcategories = array();
    if (isset($HD_SUPPORT_DEPARTMENTS) && is_array($HD_SUPPORT_DEPARTMENTS)) foreach ($HD_SUPPORT_DEPARTMENTS as $v) {
      if (in_array($v["id"], explode(",", $jakclient->getVar("support_dep")))) {
        $allcategories[] = $v;
      }       
    }

    // Similar
    $similarart = $jakdb->select($jaktable2, ["id", "subject"], ["OR #sim" => ["AND #public" => ["id[!]" => $page2, "depid" => [$jakclient->getVar("support_dep")], "content[~]" => $titlearray, "private" => 0], "AND #private" => ["id[!]" => $page2, "depid" => [$jakclient->getVar("support_dep")], "content[~]" => $titlearray, "private" => 1, "clientid" => JAK_CLIENTID]], "LIMIT" => 5]);
  ...
  if ($jakuser->getVar("support_dep") == 0 || JAK_SUPERADMINACCESS) {
    ...
    // Similar
    $similarart = $jakdb->select($jaktable2, ["id", "subject"], ["AND" => ["id[!]" => $page2, "content[~]" => $titlearray], "LIMIT" => 5]);

    // Page Navigation
    $nextp = $jakdb->get($jaktable2, ["id", "subject"], ["AND" => ["id[>]" => $page2], "ORDER" => ["id" => "ASC"]]);
    $prevp = $jakdb->get($jaktable2, ["id", "subject"], ["AND" => ["id[<]" => $page2], "ORDER" => ["id" => "DESC"]]);
    ...
    $allcategories = array();
    if (isset($HD_SUPPORT_DEPARTMENTS) && is_array($HD_SUPPORT_DEPARTMENTS)) foreach ($HD_SUPPORT_DEPARTMENTS as $v) {
      if (in_array($v["id"], explode(",", $jakuser->getVar("support_dep")))) {
        $allcategories[] = $v;
      }       
    }

    // Similar
    $similarart = $jakdb->select($jaktable2, ["id", "subject"], ["AND" => ["id[!]" => $page2, "depid" => [$jakuser->getVar("support_dep")], "content[~]" => $titlearray], "LIMIT" => 5]);
...
// The ticket is private and we are not the owner
  ...
  if (!$jakdb->has($jaktable3, ["guesta" => 1])) jak_redirect(JAK_rewrite::jakParseurl(JAK_SUPPORT_URL));
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!$result) {
          ...
          if ($jkp['jak_status'] == 3) {
            ...
            if (JAK_TICKET_CLOSE_R == 1) {
              ...
              // Dashboard URL
              $ticketurl = JAK_rewrite::jakParseurl(JAK_SUPPORT_URL, 't', $page2, JAK_rewrite::jakCleanurl($JAK_FORM_DATA["subject"]));
              ...
              if (!empty($ticktext)) {
                ...
                if (JAK_SMTP_MAIL) {
                  ...
                  $mail->Subject = JAK_TITLE.' - '.sprintf($jkl['hd101'], $JAK_FORM_DATA['subject']);
                  $mail->MsgHTML($body);
        ...
        if (JAK_USERID) {
          ...
          if (!empty($HD_ANSWERS) && is_array($HD_ANSWERS)) foreach ($HD_ANSWERS as $v) {
            ...
            if ($v["msgtype"] == 21 && $v["lang"] == JAK_LANG) {
              ...
              $replace   = array(BASE_URL, JAK_TITLE, $JAK_FORM_DATA['email'], $JAK_FORM_DATA['name'], $JAK_FORM_DATA['clientcredits'], $JAK_FORM_DATA['paid_until'], '#'.$page2, $JAK_FORM_DATA['subject'], $ticketurl, $answeremail, replace_urls_emails($contentf, BASE_URL, JAK_FILES_DIRECTORY));
              $ticktext = str_replace($phold, $replace, $v["message"]);
              break;
          ...
          if (!empty($ticktext)) {
            ...
            $body = str_ireplace("[\]", "", $nlcontent);
              ...
              $mail->SetFrom(JAK_EMAIL);
              $mail->addAddress($JAK_FORM_DATA['email']);
              $mail->Subject = JAK_TITLE.' - RE:'.$JAK_FORM_DATA['subject'];
              $mail->MsgHTML($body);
  ...
  if (JAK_USERID) {
    $JAK_ANSWER_DATA = $jakdb->select($jaktable4, ["[>]".$jaktable6 => ["operatorid" => "id"], "[>]".$jaktable5 => ["clientid" => "id"]], ["ticket_answers.id", "ticket_answers.content", "ticket_answers.lastedit", "ticket_answers.sent", "user.id(oid)", "user.name(oname)", "clients.id(cid)", "clients.name(cname)"], ["ticket_answers.ticketid" => $page2, "ORDER" => ["ticket_answers.sent" => "ASC"]]);
  } else {
    $JAK_ANSWER_DATA = $jakdb->select($jaktable4, ["[>]".$jaktable6 => ["operatorid" => "id"], "[>]".$jaktable5 => ["clientid" => "id"]], ["ticket_answers.id", "ticket_answers.content", "ticket_answers.lastedit", "ticket_answers.sent", "user.id(oid)", "user.name(oname)", "clients.id(cid)", "clients.name(cname)"], ["AND" => ["ticket_answers.ticketid" => $page2, "ticket_answers.private" => 0], "ORDER" => ["ticket_answers.sent" => "ASC"]]);
  }
  ...
  if ($ticketwrite) {
      ...
      $custom_fields = jak_get_custom_fields($JAK_CUSTOM_FIELD, 2, $JAK_FORM_DATA["depid"], $langcust, false, false, true, false, $errors);
  }
