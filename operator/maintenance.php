...
if (isset($jkp['delTFiles'])) {

	// Now let us delete the all the ticket attachments
	$ticketallfiles = APP_PATH.JAK_FILES_DIRECTORY.'/support/';
	$msfi = glob($ticketallfiles."*");
	if ($msfi) foreach ($msfi as $filen) {
		if (is_dir($filen)) {
			// Get the ticket id
			$ticketid = substr($filen, -1);
			if (is_numeric($ticketid)) {
				// Remove all files
				array_map('unlink', glob("$filen/*.*"));
				// Remove the directory
				rmdir($filen);
				// Set the attachment counter to zero
				$jakdb->update("support_tickets", ["attachments" => 0], ["id" => $ticketid]);
			}
		}
	...
	// Write the log file each time someone login after to show success
    JAK_base::jakWhatslog('', JAK_USERID, 0, 14, 0, (isset($_COOKIE['WIOgeoData']) ? $_COOKIE['WIOgeoData'] : ''), $jakuser->getVar("username"), $_SERVER['REQUEST_URI'], $ipa, $valid_agent);
		...
		// Now let's check the license
	  	...
	  	if ($activate_response['status'] != true) { 
	    	$errors['e1'] = $activate_response['message'];
	  		...
	  		$jakdb->update("settings", ["used_value" => filter_var($_POST['jak_licusr'], FILTER_SANITIZE_STRING)], ["varname" => "o_number"]);
