...
switch ($page1) {
	...
	case 'droppoints':
		...
		switch ($page2) {
			...
			default:
				...
				if (JAK_SUPERADMINACCESS) {
					$JAK_USER_ALL = jak_get_user_dp($jaktable, JAK_USERID, JAK_SUPERADMIN);				
				} elseif (jak_get_access("usrmanage", $jakuser->getVar("permissions"), JAK_SUPERADMINACCESS)) {
					$JAK_USER_ALL = jak_get_user_dp($jaktable, JAK_USERID, JAK_SUPERADMIN);					
				} else {
					$JAK_USER_ALL = jak_get_user_dp($jaktable, JAK_USERID, '');
				}
				...
				// Title and Description
				$SECTION_TITLE = 'Droppoints';
				$SECTION_DESC = "";
				
				// Include the javascript file for results
				$js_file_footer = 'js_droppoints.php';
				
				// Call the template
				$template = 'droppoints.php';
			break;
		}
	break;
	...
	case 'edit':
		...
		if (is_numeric($page2) && jak_row_exist($page2,$jaktable)) {
			...
			$JAK_DEPARTMENTS = $jakdb->select($jaktable2, ["id", "title"], ["ORDER" => ["dorder" => "ASC"]]);
			$JAK_DEP_SUPPORT = $jakdb->select($jaktable4, ["id", "title"], ["ORDER" => ["dorder" => "ASC"]]);
			$JAK_PRIORITY = $jakdb->select($jaktable9, ["id", "title"], ["ORDER" => ["dorder" => "ASC"]]);
