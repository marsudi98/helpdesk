	...
	default:
		...
		if (jak_get_access("ochat_all", $jakuser->getVar("permissions"), JAK_SUPERADMINACCESS)) {
  	 	...
  	 	if ($totalAll != 0) {
  	 		...
  	 		$CHATS_ALLC = $jakdb->select($jaktable, ["[>]user" => ["fromid" => "id"], "[>]user(to)" => ["toid" => "id"]], ["operatorchat.id", "operatorchat.fromid", "operatorchat.toid", "operatorchat.message", "operatorchat.sent", "user.username", "to.username(touser)"], ["LIMIT" => $chats->limit]);
