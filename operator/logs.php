 ...
 switch ($page1) {
	...
	default:
	    ...
	    if (isset($totalAll) && $totalAll != 0) {
			...
			// Get the busies client
			$busy_client = $jakdb->query("SELECT COUNT(t1.clientid) AS totalCL, t2.email FROM ".JAKDB_PREFIX."whatslog AS t1 LEFT JOIN ".JAKDB_PREFIX."clients AS t2 ON(t1.clientid = t2.id) WHERE t1.clientid != 0 GROUP BY t1.clientid ORDER BY totalCL DESC LIMIT 1")->fetch();
		}
