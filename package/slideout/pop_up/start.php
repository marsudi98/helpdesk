...
if (isset($_SESSION["select_operator"]) && is_numeric($_SESSION["select_operator"])) {
	...
	$selop = $jakdb->get("user", ["chat_dep", "name", "picture"], ["id" => $_SESSION["select_operator"]]);
	$dep_title = '-';
	if ($selop["chat_dep"] != 0 && is_numeric($selop["chat_dep"])) {
		foreach ($HD_DEPARTMENTS as $d) {
			if ($selop["chat_dep"] == $d["id"]) {
				$dep_title = $d['title'];
				$dep_direct = $selop["chat_dep"];
				break;
			}
		}
	}
} else {
	if (isset($_SESSION['widgetid'])) $online_oplist = online_operator_list($HD_DEPARTMENTS, $jakwidget['depid'], $jakwidget['opid']);
}
...
<div class="container">
	...
	<?php } else { ?>
		...
		<?php if (isset($online_oplist) && !empty($online_oplist)) { ?>
		<div class="row">
		<?php foreach ($online_oplist as $o) { ?>
		<div class="col-6">
		<div class="oplist" onclick="lcjak_slideout_operator(<?php echo $o['id'];?>, '<?php echo $_SERVER['REQUEST_URI'];?>');return false;">
			...
			<small class="text-muted"><?php echo ($o['title'] ? $o['title'] : $jkl['g88']);?></small>
		...
		<form class="jak-ajaxform" method="post" action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']);?>">
			...
			<?php if (JAK_CLIENTID) { ?>
				...
				<div class="row text-center">
					<div class="col-6">
						<div class="form-group">
							...
							<input type="text" name="name" id="name" class="form-control underlined" value="<?php echo $jakclient->getVar("name");?>" readonly>
						</div>
					</div>
					<div class="col-6">
						<div class="form-group">
							<label class="sr-only" for="email"><?php echo $jkl["g5"];?></label>
							<input type="email" name="email" id="email" class="form-control underlined" value="<?php echo $jakclient->getVar("email");?>" readonly>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<select name="department" id="department" class="form-control underlined">
							<?php foreach($online_op as $v) { if (in_array($v["id"], explode(',', $jakwidget["depid"])) || $jakwidget["depid"] == 0) { ?><option value="<?php echo $v["id"];?>"<?php if (isset($_REQUEST["department"]) && $_REQUEST["department"] == $v["id"]) { ?> selected="selected"<?php } ?>><?php echo $v["title"].((JAK_BILLING_MODE == 1 && $v["credits"] != 0) ? ' / '.sprintf($jkl['hd44'], $v["credits"]) : '');?></option><?php } } ?>
					</select>
				</div>
				<?php } else { ?>
				...
				<div class="form-group">
					...
					<input type="text" name="question" id="question" class="form-control" value="<?php if (isset($_REQUEST["question"])) echo $_REQUEST["question"];?>" placeholder="<?php echo $jkl["g71"];?>">
				</div>
				<?php } ?>
			...
			<?php } else { ?>
				...
				<div class="row text-center avatars">
					<div class="col-2">
						<label>
							<input type="radio" name="avatar" id="avatar" value="/package/slideout/avatar/standard.jpg" checked>
							<img src="<?php echo BASE_URL;?>/package/slideout/avatar/standard.jpg" class="rounded img-fluid" alt="avatar standard">
						</label>
					</div>
					<div class="col-2">
						<label>
	    					<input type="radio" name="avatar" value="/package/slideout/avatar/4.jpg">
	    					<img src="<?php echo BASE_URL;?>/package/slideout/avatar/4.jpg" class="rounded img-fluid" alt="avatar4">
	  					</label>
					</div>
					<div class="col-2">
						<label>
	    					<input type="radio" name="avatar" value="/package/slideout/avatar/2.jpg">
	    					<img src="<?php echo BASE_URL;?>/package/slideout/avatar/2.jpg" class="rounded img-fluid" alt="avatar2">
	  					</label>
					</div>
					<div class="col-2">
						<label>
	    					<input type="radio" name="avatar" value="/package/slideout/avatar/3.jpg">
	    					<img src="<?php echo BASE_URL;?>/package/slideout/avatar/3.jpg" class="rounded img-fluid" alt="avatar3">
	  					</label>
					</div>
					<div class="col-2">
						<label>
	    					<input type="radio" name="avatar" value="/package/slideout/avatar/1.jpg">
	    					<img src="<?php echo BASE_URL;?>/package/slideout/avatar/1.jpg" class="rounded img-fluid" alt="avatar">
	  					</label>
					</div>
					<div class="col-2">
						<label>
							<input type="radio" name="avatar" value="/package/slideout/avatar/5.jpg">
							<img src="<?php echo BASE_URL;?>/package/slideout/avatar/5.jpg" class="rounded img-fluid" alt="avatar5">
						</label>
					</div>
				</div>
				<?php } ?>
