...
if (isset($row) && !empty($row)) {
	...
	if (isset($message) && !empty($message) && $row['banned'] == 0) {
		...
		$msgquote = "";
		if (isset($_POST['msgquote']) && !empty($_POST['msgquote'])) $msgquote = filter_var($_POST['msgquote'], FILTER_SANITIZE_STRING);
		...
		if (file_exists($groupchatfile) && $gcfilesize > 500000) {
		...
		$cmsg = $ctime.':#!#:'.$row['id'].':#!#:'.$_SESSION['gcname'].':#!#:'.$row["usr_avatar"].':#!#:'.$message.':#!#:'.$msgquote.':!n:';
